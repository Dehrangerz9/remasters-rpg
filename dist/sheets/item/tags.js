...
export const buildTagContext = (item, system) => {
...
export const setupTagSystem = (sheet, html) => {
    ...
    if (!addButton.length || !editor.length || !input.length || !suggestions.length)
        return;
    const normalizeName = (value) => String(value ?? "").trim();
    const getTooltip = (name) => MAJOR_TAG_MAP.get(name.toLowerCase()) ?? "";
    const readTags = () => {
    ...
    const showEditor = (value = "", editIndex = null) => {
    ...
    tagRoot.find("[data-action='tag-remove']").on("click", async (event) => {
        event.preventDefault();
    ...
    tagRoot.find(".tag-pill[data-index]").on("dblclick", (event) => {
    ...
    suggestions.on("click", ".tag-suggestion", async (event) => {
        event.preventDefault();
