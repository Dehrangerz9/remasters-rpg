...
export const bindPlayerFeatListeners = (sheet, html) => {
    ...
    mainRoot.find("[data-action='feat-chat']").on("click", async (event) => {
        event.preventDefault();
    ...
    mainRoot.find("[data-action='feat-toggle-details']").on("click", (event) => {
        event.preventDefault();
    ...
    mainRoot.find("[data-action='feat-open']").on("click", async (event) => {
        event.preventDefault();
        const id = String(event.currentTarget.dataset.id ?? "");
        if (!id)
            return;
    ...
    mainRoot.find("[data-action='feat-delete']").on("click", async (event) => {
        event.preventDefault();
        const id = String(event.currentTarget.dataset.id ?? "");
        if (!id)
            return;
