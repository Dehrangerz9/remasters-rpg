...
const sanitizeFormula = (value) => String(value ?? "").trim();
const toEvaluableFormula = (value) => {
    const trimmed = sanitizeFormula(value);
    if (!trimmed)
        return "0";
    if (trimmed.startsWith("+"))
        return trimmed.slice(1).trim() || "0";
    return trimmed;
};
const getDamageTypeLabel = (damageType) => localize(DAMAGE_TYPE_META[damageType].labelKey);
const getDamageTypeBadge = (damageType) => {
...
const getDamageTypeOptions = () => Object.keys(DAMAGE_TYPE_META).map((key) => ({
    value: key,
    label: localize(DAMAGE_TYPE_META[key].labelKey)
}));
const formatBreakdown = (values) => {
    if (!values.length)
        return "(0)";
    let label = "";
...
const buildDamageEntriesFromItem = (actor, item) => {
    const entries = [];
    const modifiers = [];
    if (!item) {
    ...
    if (item.type === "weapon") {
        ...
        bonusEntries.forEach((bonus, index) => {
...
const buildFallbackDamageEntries = (formula) => {
...
const renderDamageRows = (entries) => entries
    .map((entry, index) => `
    ...
    `)
    .join("");
const renderModifierRows = (modifiers) => modifiers
    .map((modifier, index) => `
      <div class="rmrpg-damage-dialog-row">
        <div class="rmrpg-damage-dialog-cell rmrpg-damage-dialog-label">${escapeHtml(modifier.name)}</div>
        <div class="rmrpg-damage-dialog-cell rmrpg-damage-dialog-formula">${escapeHtml(modifier.type)} ${escapeHtml(formatSigned(toSafeInteger(modifier.value, 0)))}</div>
    ...
    `)
    .join("");
const buildPreviewMarkup = (entries, modifiers, criticalMultiplier) => {
    ...
    selectedModifiers.forEach((modifier) => {
        ...
        const iconMarkup = meta.icon ? ` <i class="${meta.icon}" aria-hidden="true"></i>` : "";
        pieces.push({
            html: `<span class="rmrpg-damage-preview-term rmrpg-damage-preview-${meta.css}">${escapeHtml(`${Math.abs(value)}`)}${iconMarkup}</span>`,
            negative: value < 0
        });
    });
    if (!pieces.length) {
    ...
    const multiplierLabel = Number.isFinite(criticalMultiplier) && Math.abs(criticalMultiplier - 1) > 0.0001
...
const openDamageRollDialog = async (title, initialData) => {
  ...
  `;
    return new Promise((resolve) => {
        ...
        const sync = (html) => {
            ...
            html.find("[data-role='damage-toggle']").on("change", (event) => {
            ...
            html.find("[data-role='modifier-toggle']").on("change", (event) => {
                const index = Number(event.currentTarget.dataset.index ?? -1);
                if (!Number.isFinite(index) || index < 0 || index >= modifiers.length)
                    return;
...
const evaluateDamageTerms = async (entries) => {
    const selectedEntries = entries.filter((entry) => entry.checked);
    const terms = [];
...
const buildFormulaMarkup = (terms, modifiers, criticalMultiplier) => {
    const formulaChunks = [];
    terms.forEach((term) => {
    ...
    const criticalBadge = Number.isFinite(criticalMultiplier) && Math.abs(criticalMultiplier - 1) > 0.0001
...
const buildDetailsMarkup = (result) => {
    const detailTags = [];
    result.terms.forEach((term) => {
        detailTags.push(`<span class="rmrpg-damage-detail">${escapeHtml(term.entry.label)} ${escapeHtml(formatSigned(term.total))} ${escapeHtml(getDamageTypeLabel(term.entry.damageType))}</span>`);
    });
    result.modifiers
        .filter((modifier) => modifier.checked)
        .forEach((modifier) => {
        detailTags.push(`<span class="rmrpg-damage-detail">${escapeHtml(modifier.name)} ${escapeHtml(formatSigned(toSafeInteger(modifier.value, 0)))}</span>`);
    });
...
const buildDamageChatContent = (result) => {
...
const bindDamageCardControls = (messageId, sourceActor) => {
...
export const rollDamageWithDialog = async ({ actor, item = null, fallbackFormula = "", itemName = "", sourceLabel = "", criticalMultiplier = 1 }) => {
    if (!actor)
        return null;
