...
function getEvolutionLabel(index: number) {
  const rankIndex = Math.min(RANKS.length - 1, Math.floor((index - 1) / 4));
  const step = ((index - 1) % 4) + 1;
  return `${RANKS[rankIndex]}${step}`;
}

function extractAdvancementIndex(key: string) {
  const match = key.match(/(\d+)/);
  return match ? Number(match[1]) : 0;
}

function sortAdvancementKeys(keys: string[]) {
...
export class RMRPGActorSheet extends ActorSheet {
  ...
  async getData(options = {}) {
    ...
    if (context.isPlayer) {
      ...
      const baseSkills = SKILL_DEFS.map((definition) => {
        ...
        let labelKey: string | null = definition.labelKey;
        let label = "";
      ...
      const customEntries = customSkills.map((entry: any, index: number) => {
      ...
      const mappedEquipment = equipmentItems.map((item: any) => {
        ...
        return {
          ...
          statusLabel: statusLabels[status as keyof typeof statusLabels] ?? statusLabels.stowed
        };
      });
  ...
  activateListeners(html: JQuery) {
    ...
    const bindReorderList = (listElement: HTMLElement, rowSelector: string, group: string) => {
      ...
      listElement.addEventListener("drop", async (event) => {
        ...
        const rows = Array.from(listElement.querySelectorAll(rowSelector)) as HTMLElement[];
        const draggedIndex = rows.findIndex((row) => row.dataset.id === data.id);
        if (draggedIndex === -1) return;

        const targetRow = (dragEvent.target as HTMLElement | null)?.closest(rowSelector) as HTMLElement | null;
        ...
        if (normalizedIndex > draggedIndex) normalizedIndex -= 1;
    ...
    html.find("[data-action='feat-chat']").on("click", async (event: any) => {
      event.preventDefault();
      const id = String(event.currentTarget.dataset.id ?? "");
      if (!id) return;
      const item = this.actor.items?.get(id);
    ...
    html.find("[data-action='feat-open']").on("click", async (event: any) => {
      event.preventDefault();
      const id = String(event.currentTarget.dataset.id ?? "");
      if (!id) return;
      const item = this.actor.items?.get(id);
    ...
    html.find("[data-action='feat-delete']").on("click", async (event: any) => {
      event.preventDefault();
      const id = String(event.currentTarget.dataset.id ?? "");
      if (!id) return;
      const item = this.actor.items?.get(id);
  ...
  private setItemDragData(event: DragEvent, data: { id: string; group: string }) {
  ...
  private readItemDragData(event: DragEvent) {
    const raw = event.dataTransfer?.getData("application/x-rmrpg-item");
    if (!raw) return null;
    try {
      const parsed = JSON.parse(raw);
      if (!parsed?.id || !parsed?.group) return null;
      return parsed as { id: string; group: string };
    } catch {
      return null;
    }
  }

  private async updateItemSortOrder(ids: string[]) {
  ...
  private bindInlineIntegerInput(html: JQuery, selector: string, onCommit: (value: number) => Promise<void>) {
  ...
  private parseNumericInput(rawValue: string) {
  ...
  private async openAttributeDialog() {
  ...
  private async openDefenseDialog() {
  ...
  private async openSkillSettings() {
    if (this.actor.type !== "player") return;
    ...
    const dialog = await this.showDialog(
      game.i18n.localize("RMRPG.Actor.Skills.NamesTitle"),
      `
      <form class="rmrpg-dialog-form">
        <div class="form-group">
  ...
  private async onFeatDrop(event: DragEvent) {
    event.preventDefault();
    if (!this.actor) return;

    const reorderData = this.readItemDragData(event);
    if (reorderData?.group === "feats") return;
  ...
  private async sendFeatToChat(item: any) {
  ...
  private async showDialog(title: string, content: string): Promise<JQuery | null> {
  ...
  private readDialogRawValue(dialogHtml: JQuery, inputName: string) {
    return String(dialogHtml.find(`[name='${inputName}']`).val() ?? "").trim();
  }

  private readDialogNumber(dialogHtml: JQuery, inputName: string, fallback: number) {
