...
type RollCheckOptions = {
  actor: any;
  label: string;
  totalModifier: number;
  dc?: number | null;
  targetInfoLabel?: string;
  passosDeSucessos?: number;
  passosDeFalha?: number;
...
const OUTCOME_LEVELS: CheckOutcome[] = ["critical-failure", "failure", "success", "critical-success"];
const OUTCOME_TO_LEVEL = new Map<CheckOutcome, number>(
  OUTCOME_LEVELS.map((outcome, index) => [outcome, index])
);

const OUTCOME_LABEL_KEY: Record<CheckOutcome, string> = {
...
const clampOutcomeLevel = (value: number) => {
  if (!Number.isFinite(value)) return 0;
  return Math.max(0, Math.min(OUTCOME_LEVELS.length - 1, Math.floor(value)));
};

const toStepValue = (value: unknown) => {
  const parsed = Number(value ?? 0);
  if (!Number.isFinite(parsed)) return 0;
  return Math.max(0, Math.floor(parsed));
};

const normalizeOptionalDc = (value: unknown) => {
...
const extractNaturalD20 = (roll: any) => {
  const d20 = Array.isArray(roll?.dice) ? roll.dice.find((die: any) => Number(die?.faces) === 20) : null;
  if (!d20) return null;
  const activeResult = Array.isArray(d20.results) ? d20.results.find((entry: any) => entry?.active !== false) : null;
  const value = Number(activeResult?.result ?? d20.total);
  return Number.isFinite(value) ? Math.floor(value) : null;
};

const resolveBaseOutcome = (total: number, dc: number, naturalD20: number | null): CheckOutcome => {
  if (naturalD20 === 1) return "critical-failure";
  if (naturalD20 === 20 || total >= dc + 5) return "critical-success";
  if (total >= dc) return "success";
  return "failure";
};

const applyOutcomeSteps = (base: CheckOutcome, passosDeSucessos: number, passosDeFalha: number, naturalD20: number | null) => {
  // Natural 1 and natural 20 always lock the extreme result.
  if (naturalD20 === 1 || naturalD20 === 20) return base;
...
const buildFormulaLabel = (totalModifier: number) => {
  if (totalModifier === 0) return "1d20";
  return `1d20 ${totalModifier > 0 ? "+" : "-"} ${Math.abs(totalModifier)}`;
};

const getDiceRows = (roll: any) => {
  const rows: Array<{ formula: string; values: number[]; total: number }> = [];
  ...
  for (const die of dice) {
    ...
    const values = Array.isArray(die?.results)
      ? die.results
          ...
          .map((entry: any) => Number(entry?.result))
...
const buildCheckChatContent = (params: {
    ...
    dc !== null && outcome
      ? `
        <div class="rmrpg-check-outcome ${
          outcome === "success" || outcome === "critical-success" ? "is-success" : "is-failure"
        }">
          ...
          ${
            hideTargetInfoAsSecret && targetInfoLabel
              ? `<section class="secret rmrpg-check-outcome-secret"><span class="rmrpg-check-outcome-dc">(${escapeHtml(
                  targetInfoLabel
                )})</span></section>`
  ...
  const damageButtonMarkup =
    damageButton && String(damageButton.formula ?? "").trim()
      ? `
...
export const rollSkillOrDerivedCheck = async ({
  actor,
  label,
  totalModifier,
  dc = null,
