...
export const openRollDialog = async (title: string, modifiers: RollModifier[], allowCustom = true) => {
  ...
  return new Promise<{ total: number; modifiers: RollModifier[] } | null>((resolve) => {
    let resolved = false;

    const updateRollLabel = (html: JQuery) => {
    ...
    const bindToggleEvents = (html: JQuery) => {
      html.find(".modifier-toggle").on("change", (event: any) => {
