...
export const syncAbilitiesForCategoryEffect = async (item: any) => {
  ...
  const applyAbilityUpdate = async (abilityItem: any) => {
    const ability = abilityItem.system?.ability;
    if (!ability || !Array.isArray(ability.categories)) return;
    const categorySystem = item.system ?? {};
    const updatedCategories = ability.categories.map((entry: any) => {
      if (String(entry?.uuid ?? "") !== uuid) return entry;
      return {
        ...entry,
        uuid,
    ...
    if (!updatedCategories.some((entry: any) => String(entry?.uuid ?? "") === uuid)) return;
  ...
  const worldAbilities = game.items?.filter((entry: any) => entry.type === "ability") ?? [];
  ...
  for (const actor of actors) {
    const abilityItems = actor.items?.filter((entry: any) => entry.type === "ability") ?? [];
  ...
  for (const app of Object.values(windows)) {
    if (!app?.item || app.item.type !== "ability") continue;
    const categories = app.item.system?.ability?.categories ?? [];
    if (!Array.isArray(categories)) continue;
    if (categories.some((entry: any) => String(entry?.uuid ?? "") === uuid)) {
      app.render(false);
    }
  }
};
